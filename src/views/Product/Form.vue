    ...
    <b-card header="Product Form">
      <b-form @submit="onSubmit" v-if="show">
        ...
        <b-form-group id="input-group-2" label="Description" label-for="description">
          <b-form-input id="description" v-model="formData.description" required></b-form-input>
        </b-form-group>

        <b-form-group id="input-group-2" label="Price" label-for="price">
          <b-form-input id="price" v-model="formData.price" required></b-form-input>
        </b-form-group>

        <b-form-group id="input-group-1" label="Category Name" label-for="name">
          <Choices
            :options="categories"
            v-on:input="formData.categories = $event"
            v-bind:value="formData.categories"
          />
        </b-form-group>
      </b-form>
    </b-card>
